...
def mapper():
    ...
    for line in reader:
        ...
        if len(line) == 19:
            ...
            extra,extra_ref_id,extra_count,marked = line
        if id == "id":
            continue
        temp = [author_id,'B',id,title,author_id,tagnames,node_type,parent_id,abs_parent_id,added_at,score]

        if len(line) == 5:
            user_ptr_id,reputation,gold,silver,bronze = line
        if user_ptr_id == "user_ptr_id":
            continue
        temp = [user_ptr_id,'A',reputation,gold,silver,bronze]
